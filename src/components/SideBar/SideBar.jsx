import { Heading, HStack, Icon, Link, VStack, useBreakpointValue, Text } from "@chakra-ui/react"
      ...
      <VStack w="full" alignItems={alignItems}>
        ...
        <ClientOnly>
          {showText && <VStack  alignItems='flex-start'>
            {bottomLinks?.map(link => <Link key={`${link.url}${link.text}bottomLink`} href={link.url} as={NextLink}><Text color="gray.600">{link.text}</Text></Link>)}
          </VStack>}
        </ClientOnly>

        <HStack pt={4} justifyContent={{base:'center', md:'space-between'}} w="full" alignItems="center">
          {showText && <Heading size="lg" fontWeight='semibold' color="gray.600">Curated</Heading>}
